...
<div class="flex flex-col items-center">
  ...
  <form action="{{ url('/ingredient/' . $ingredient->id) }}" method="POST" class="md:w-1/2 w-full">
    @csrf
      ...
      <div class="px-4 py-5 w-100 bg-white space-y-6 sm:p-6">
        ...
        <div>
          <div class="col-span-3 sm:col-span-2">
            ...
            <div class="mt-1 flex flex-col rounded-md">
              <input required type="text" name="name" id="ingredient-name" class="shadow-sm @error('name') is-invalid @enderror p-1 border focus:ring-indigo-500 focus:border-indigo-500 flex-1 block w-full rounded-md sm:text-lg border-gray-300" placeholder="Flour" value="{{ old('name', $ingredient->name) }}">
              @error('name')
        ...
        <div>
          ...
          <div class="mt-1 grid grid-cols-11 gap-2">
            <input step="any" required type="number" name="harga_bahan" id="ingredient-price" class="col-span-5 shadow-sm @error('harga_bahan') is-invalid @enderror p-1 border focus:ring-indigo-500 focus:border-indigo-500 flex-1 block w-full rounded-md sm:text-lg border-gray-300" placeholder="20000" value="{{ old('harga_bahan', $ingredient->harga_bahan) }}">
            @error('harga_bahan')
              <span class="invalid-feedback" role="alert">
                  <strong>{{ $message }}</strong>
              </span>
        ...
        <div>
          ...
          <div class="mt-1 grid grid-cols-2 gap-2">
            <input required type="number" name="stock" id="ingredient-stock" class="col-span-1 shadow-sm @error('stock') is-invalid @enderror p-1 border focus:ring-indigo-500 focus:border-indigo-500 flex-1 block w-full rounded-md sm:text-lg border-gray-300" placeholder="100" value="{{ old('stock', $ingredient->stock) }}">
            @error('stock')
              <span class="invalid-feedback" role="alert">
                  <strong>{{ $message }}</strong>
              </span>
            ...
            <select required name="unit" id="ingredient-unit" class="col-span-1 shadow-sm @error('unit') is-invalid @enderror p-1 border focus:ring-indigo-500 focus:border-indigo-500 flex-1 block w-full rounded-md sm:text-lg border-gray-300">
              ...
              <option value="Kilogram" {{ old('unit', $ingredient->unit) == 'Kilogram' ? 'selected' : '' }}>Kilogram</option>
        ...
        <div class="px-4 py-3 text-right sm:px-6">
          <button type="submit" class="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-lg font-medium rounded-md text-white bg-yellow-500 hover:bg-yellow-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">Edit</button>
        </div>
      </div>
    </div>
  </form>
